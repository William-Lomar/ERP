  ...
  class Painel{
    ...
    public static function selectFilter($tabela,$local,$tipoEmpresa){
      ...
      if ($tipoEmpresa == 'geral' && $local == 'total') {
            ...
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE local = 'bancada'");
            $sql->execute();    
          }elseif (($tipoEmpresa == 'iema' || $tipoEmpresa == 'empresa') && $local == 'total') {
            # code...
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE empresa = ?");
            $sql->execute(array($tipoEmpresa));
          }elseif (($tipoEmpresa == 'iema' || $tipoEmpresa == 'empresa') && $local == 'almoxarifado') {
            # code...
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE empresa = ? AND local <> 'bancada'");
            $sql->execute(array($tipoEmpresa));
          }elseif (($tipoEmpresa == 'iema' || $tipoEmpresa == 'empresa') && $local == 'bancada') {
            # code...
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE empresa = ? AND local = 'bancada'");
            $sql->execute(array($tipoEmpresa));
          }elseif($tipoEmpresa == 'clientes' && $local == 'total'){
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE empresa <> 'IEMA' AND empresa <> 'empresa'");
            $sql->execute(); 
          }elseif($tipoEmpresa == 'clientes' && $local == 'almoxarifado'){
            $sql = MySql::conectar()->prepare("SELECT * FROM `$tabela` WHERE empresa <> 'IEMA' AND empresa <> 'empresa' AND local <> 'bancada'");
            $sql->execute(); 
